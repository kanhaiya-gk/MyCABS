...
class VehicleRateCategory(models.Model):
    ...
    tariff_per_min = models.PositiveIntegerField()
    tariff_after_kms = models.PositiveIntegerField()
...
class Rate(models.Model):
    ...
    oneway_time = models.PositiveIntegerField()
    oneway_distance = models.PositiveIntegerField(default=0, blank=True)

    roundtrip_time = models.PositiveIntegerField(blank=True, default=0)
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.roundtrip_time:
            self.roundtrip_time = 2 * self.oneway_time
        if not self.roundtrip_distance:
            self.roundtrip_distance = 2 * self.oneway_distance
        super().save(*args, **kwargs)

    @property
    def total_oneway_price(self):
        return int(round(self.oneway_time * (int)(self.vehicle_category.tariff_per_min)))

    @property
    def total_roundtrip_price(self):
        return int(round(self.roundtrip_time * (int)(self.vehicle_category.tariff_per_min) ))
...
class Booking(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.id is None:
            ...
            fare_details = {
                'tariff_per_min': self.vehicle_type.tariff_per_min,
                'after_km_charges': self.vehicle_type.tariff_after_kms,
                'price': (rate.oneway_time * self.vehicle_type.tariff_per_min if self.booking_type == 'OW' else
                          rate.roundtrip_time * self.vehicle_type.tariff_per_min),
                'discount': 0,
                'markup': 0
            }
        else:
